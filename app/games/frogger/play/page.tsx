...
declare global {
  interface Window {
    FROGGER: {
      start(canvas: HTMLCanvasElement, callbacks: FroggerCallbacks): FroggerAPI;
    };
  }
}
...
export default function FroggerPlayPage() {
  ...
  const apiRef = useRef<FroggerAPI | null>(null);
  ...
  const [skin, setSkinState] = useState<SkinName>(() => {
    ...
    const saved = window.localStorage.getItem("arcade-skin-frogger");
    ...
    function mount() {
      ...
      intervalId = setInterval(() => {
        if (window.FROGGER) {
          ...
          apiRef.current = window.FROGGER.start(canvas, callbacks);
        }
      }, 50);

      timeoutId = setTimeout(() => {
        clearInterval(intervalId);
        console.error("Frogger: window.FROGGER not available after 5s");
      }, 5000);
    }
  ...
  function handleSave() {
    saveScore({ game: "frogger", score: finalScore, name });
    setSaved(true);
  }

  function handleRestart() {
    const canvas = canvasRef.current;
    if (!canvas || !window.FROGGER) return;
  ...
  return (
    <>
      <Script src="/games/frogger.js" strategy="afterInteractive" />
      ...
      <div className="av-player fade-in">
        <div className="player-hud">
          ...
          <div className="hud-actions">
            ...
            <button className="btn ghost" onClick={() => router.push("/games/frogger")}>SALIR</button>
          </div>
        </div>
          ...
          <div className="modal-bd">
            <div className="modal">
              ...
              <div className="actions">
                ...
                <button className="btn magenta" onClick={() => router.push("/games/frogger")}>VOLVER AL VAULT</button>
              </div>
            </div>
          </div>
        )}
