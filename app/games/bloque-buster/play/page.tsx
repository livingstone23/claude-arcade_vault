...
const game = GAMES.find((g) => g.id === "bloque-buster")!;

export default function BloqueBusterPlayPage() {
  const router = useRouter();
  const { user, saveScore } = useUser();
  const isTouch = useTouchDevice();

  const canvasRef = useRef<HTMLCanvasElement>(null);
  const apiRef = useRef<BloqueBusterAPI | null>(null);
  ...
  useEffect(() => {
    ...
    const callbacks: BloqueBusterCallbacks = {
    ...
    function mount() {
      ...
      timeoutId = setTimeout(() => {
        clearInterval(intervalId);
        console.error("BloqueBuster: window.BLOQUE_BUSTER not available after 5s");
      }, 5000);
    }
  ...
  function handleRestart() {
    const canvas = canvasRef.current;
    if (!canvas || !window.BLOQUE_BUSTER) return;
    apiRef.current?.destroy();
  ...
  return (
    <>
      <Script src="/games/bloque-buster.js" strategy="afterInteractive" />

      {isTouch && <RotateDeviceHint />}
      {isTouch && !over && !paused && (
        <TouchControls
          config={{
            dpad: { left: { key: "ArrowLeft", code: "ArrowLeft" }, right: { key: "ArrowRight", code: "ArrowRight" } },
          }}
        />
      )}
      ...
      <div className="av-player fade-in">
        <div className="player-hud">
          ...
          <div className="hud-actions">
            ...
            <button className="btn ghost" onClick={() => router.push("/games/bloque-buster")}>SALIR</button>
          </div>
        </div>
          ...
          <div className="modal-bd">
            <div className="modal">
              ...
              <div className="actions">
                ...
                <button className="btn magenta" onClick={() => router.push("/games/bloque-buster")}>VOLVER AL VAULT</button>
              </div>
            </div>
          </div>
        )}
